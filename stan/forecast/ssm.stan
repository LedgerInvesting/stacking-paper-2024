...
transformed parameters {
  ...
  for(i in 1:N){
    for(j in 1:T_train){
      real lag = mu[i];
      mu[i] = (j == 1) ? init[i] : (1 - phi[i]) * (alpha + u[i, 1]) + phi[i] * lag;
      lp[i][j] = gamma2_lpdf(loss_ratio_train[i][j] | exp(mu[i]), exp(sigma[i]));
    }
  }
}
...
generated quantities {
  ...
  for(i in 1:N){
    real mu_pred;
    for(j in 1:T_test){
      real lag = (j == 1) ? mu[i] : mu_pred;
      mu_pred = (1 - phi[i]) * (alpha + u[i, 1]) + phi[i] * lag;
      log_lik[i][j] = gamma2_lpdf(loss_ratio_test[i][j] | exp(mu_pred), exp(sigma[i]));
      post_pred[i][j] = gamma2_rng(exp(mu_pred), exp(sigma[i]));
    }
  }
}
