...
transformed parameters {
  matrix[N, T_train] lp;
  matrix[N, 2] u = (diag_pre_multiply(sigma_u, L_omega) * Z)';

  for(i in 1:N){
    for(j in 1:T_train){
      real mu = alpha + u[i, 1] + (beta + u[i, 2]) * AY_train_z[i][j];
      lp[i][j] = gamma2_lpdf(loss_ratio_train[i][j] | exp(mu), exp(sigma));
    }
  }
}
...
generated quantities {
  ...
  for(i in 1:N){
    for(j in 1:T_test){
      ...
      log_lik[i][j] = gamma2_lpdf(loss_ratio_test[i][j] | exp(mu), exp(sigma));
      post_pred[i][j] = gamma2_rng(exp(mu), exp(sigma));
    }
  }
}
